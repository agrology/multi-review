...
cmd_pr_copy() {
  ...
  # With -a, raw binary is inlined into the copy, and its NUL bytes make grep-based controls
  # blind while awk still sees the lines — `channel-check` then passes VACUOUSLY, reopening #32
  # (proven A/B: clean copy rc=1, one-NUL copy rc=0). It also inflated a 125 B diff to 402 KB,
  # which `_payload_guard` cannot catch because -a inflates both sides equally (fable-rd3-r3,
  # -r4). The .gitattributes vector stays OPEN and tracked rather than traded for a worse one.
  ...
  git -C "$root" diff --no-ext-diff --no-textconv "$mb" "$head" > "$tmp/full" 2>/dev/null \
