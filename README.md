...
|---|---|---|
| `fable` *(on by default)* | anthropic | **none** — runs in-harness; if a fable dispatch fails before it writes (e.g. out of credits), the slot falls back to `opus` for the rest of the review, recorded as an observation at the gate |
...
| `MULTI_REVIEW_REVIEWER_MODEL` | *(provider default)* | pin a provider's model (`codex`→`gpt-6-astra`, `fable`→`fable`, `gemini`→`gemini-pro-latest`) |
...
- `scripts/multi-review-star.sh` — star grammar: mode / resolve-set / merge / open-findings / check-converged / gate-summary / witness-gaps / refan-check / release / blind-check / channel-check / compose
